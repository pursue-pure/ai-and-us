...
覆盖：创建会话 → 移动 → 攻击（普通命中）→ 无敌人攻击（边界）→ 无效方向移动（边界）→ 死亡复活
"""
import json
...
# ── 3. 无效方向（边界测试）───────────────────────────────
print("\n[3] 向上移动（无效方向，边界测试）")
status, body = req("POST", f"/session/{SESSION}/move", {"direction": "up"})
...
# ── 5. 无敌人房间攻击（边界测试）────────────────────────
print("\n[5] 无敌人房间攻击（边界测试）")
# Prism Mock 按契约示例返回，此处验证 attack 响应结构符合契约
status, body = req("POST", f"/session/{SESSION}/attack")
check("POST /attack 响应结构含 enemy_alive 字段", "enemy_alive" in body)
check("POST /attack 响应结构含 player_dead 字段", "player_dead" in body)
