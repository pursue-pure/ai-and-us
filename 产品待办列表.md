...
> 说明：本清单已按当前仓库代码与测试状态更新。部分 Sprint 2 目标已落地（如 `CombatService`、`JsonSaveRepository`、`GameSnapshot`、CI 复杂度门禁），未完成项继续保留为后续待办。

---

## 一、遗留任务总览

| 编号 | 任务名称 | 来源 | 优先级 | 状态 | 负责人 | 预计完成时间 |
|------|----------|------|--------|------|--------|--------------|
| PB-01 | 提取 `CombatService`，拆解 `attack_enemy()` 上帝方法 | Sprint 2 DDS §3.2 | 高 | ✅ 已完成 | 曹睿杰 | Sprint 3 |
| PB-02 | 修复 US-006：存档读档未完整恢复房间状态（敌人 / 物品） | 用户故事池 US-006 | 高 | ✅ 已完成 | 张津毓 | Sprint 3 |
| PB-03 | 提取 `JsonSaveRepository`，解耦持久化与引擎 | Sprint 2 DDS §3.5 | 高 | ✅ 已完成 | 张津毓 | Sprint 3 |
| PB-04 | 提取 `CheckpointService`，解耦复活与检查点逻辑 | Sprint 2 DDS §3.4 | 高 | ✅ 已完成 | 曹睿杰 | Sprint 3 |
| PB-05 | 补全核心服务层单元测试（战斗 / 复活 / 存读档分支） | Sprint 2 DDS §6.3 | 高 | 🔄 持续补强 | 张津毓 | Sprint 3 |
| PB-06 | 提取 `InventoryService`，消除 `item_type` 多处重复分支 | Sprint 2 DDS §3.3 | 中 | ⏳ 未开始 | 曹睿杰 | Sprint 3 |
| PB-07 | 引入 `LegacyCommandAdapter`，隔离命令层与新服务层 | Sprint 2 DDS §2.1 | 中 | ⏳ 未开始 | 王鹏 | Sprint 3 |
| PB-08 | 引入 `WorldBuilder` 工厂，统一世界初始化与读档重建 | Sprint 2 DDS §3.6 | 中 | ⏳ 未开始 | 王鹏 | Sprint 3 |
| PB-09 | 建立 `GameSnapshot` 统一快照模型 | Sprint 2 回顾报告 §5 | 中 | ✅ 已完成 | 张津毓 | Sprint 3 |
| PB-10 | 引入策略模式：`CombatResolutionStrategy` / `ItemEffectStrategy` | Sprint 2 DDS §2.3 | 低 | ⏳ 未开始 | 曹睿杰 | 后续迭代 |
| PB-11 | 引入 `GameContext` 单例，统一运行时上下文与仓储注册 | Sprint 2 DDS §2.3 | 低 | ⏳ 未开始 | 王鹏 | 后续迭代 |
| PB-12 | CI 集成 McCabe 复杂度门禁（单方法阈值 ≤ 7） | Sprint 2 回顾报告 §5 | 低 | ✅ 已完成 | 王鹏 | 后续迭代 |

### 本轮核查后的完成说明（2026-04-22）

- `PB-01`：`game/services/combat_service.py` 已落地，`GameEngine.attack_enemy()` 已改为委托服务。
- `PB-02`：读档已恢复敌人与房间物品状态，相关测试已覆盖。
- `PB-03`：`game/infrastructure/json_save_repository.py` 已承接 JSON 存读档职责。
- `PB-04`：`game/services/checkpoint_service.py` 已落地，`GameEngine.respawn()` 与检查点元数据已改为委托服务处理。
- `PB-05`：已新增回归测试并维持全量通过，但未达到“新增 ≥15 个”目标，当前处于持续补强阶段。
- `PB-09`：`game/snapshot.py` 已提供统一快照模型。
- `PB-12`：`.github/workflows/ci.yml` 已包含 `radon + xenon` 复杂度门禁。
...
需在存档 JSON 的 `rooms` 节点中补充 `enemy_hp` 和 `items` 字段，并在 `load_game()` 中完整恢复这两项状态。此修复已随 PB-03 迁移至 `JsonSaveRepository`。
...
- `save(filename: str, snapshot: GameSnapshot)` — 序列化并写入文件
- `load(filename: str) -> GameSnapshot` — 读取文件并反序列化为快照对象
- 文件不存在、JSON 结构损坏时抛出明确的 `SaveLoadError` 领域异常

`GameEngine.save_game()` / `load_game()` 已重构为调用仓储并处理异常的薄包装。

**优先级判定依据**  
PB-02 的 bug 修复需要扩展存档结构，若不同步解耦持久化层，修复代码仍会继续堆积在 `GameEngine` 中，形成新的耦合。两项任务在 Sprint 3 中应配合推进。

**验收标准**
- [x] `JsonSaveRepository` 可独立实例化和测试，不依赖 `GameEngine`
- [x] `GameEngine` 中不再包含任何 `json.dump` / `json.load` 直接调用
- [x] 合法存档、缺失文件、结构损坏三类场景均有对应测试
...
截至当前代码状态，Sprint 3 已完成战斗服务解耦、存档仓储解耦、快照模型落地与 CI 复杂度门禁，下一阶段应集中完成 `CheckpointService`、`InventoryService`、`LegacyCommandAdapter`、`WorldBuilder` 以及 `PB-05` 的剩余测试缺口。
